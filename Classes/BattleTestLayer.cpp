...
//初始化单位选择面板
void BattleTestLayer::setupUnitSelectionPanel()
{
	auto visibleSize = Director::getInstance()->getVisibleSize();
	auto origin = Director::getInstance()->getVisibleOrigin();

	// 创建面板（传入选择改变的回调）
	unit_selection_panel_ = UnitSelectionPanel::create(
		[this](UnitType type)
		{
			// 当选择改变时，更新当前选中的单位类型
			current_selected_unit_ = type;
			//关键：重置双击状态
			this->resetClickState();
			CCLOG("Current unit changed to: %d", (int)type);
			...
			if (coordinates_label_)
			{
				...
				coordinates_label_->setString(
					"Double-click to place: " + unitName
				);
			}
		}
	);

	// 固定在屏幕底部（不受game_world缩放影响）
	unit_selection_panel_->setPosition(Vec2(origin.x, origin.y));
	this->addChild(unit_selection_panel_, 5000); // 高层级，确保在最上面

	// 添加可用单位图标
	// 注意：这里需要替换成你实际的图片路径
	unit_selection_panel_->addUnitIcon(
		"Barbarian.png",  // 替换成你的图标路径
		"Barbarian",
		UnitType::BARBARIAN
	);

	unit_selection_panel_->addUnitIcon(
		"Archer.png",     // 替换成你的图标路径
		"Archer",
		UnitType::ARCHER
	);
	...
	unit_selection_panel_->addUnitIcon(
		"Goblin.png",      // 替换成你的图标路径
		"Goblin",
		UnitType::GOBLIN
	);

	unit_selection_panel_->addUnitIcon(
		"Bomber.png",      // 替换成你的图标路径
		"Bomber",
		UnitType::BOMBER
	);
}
...
{
	auto config = CombatSessionManager::getInstance(); // 获取单例
	auto visibleSize = Director::getInstance()->getVisibleSize();
	auto origin = Director::getInstance()->getVisibleOrigin();
...
void BattleTestLayer::placeDefender()
{
	...
	for (const auto& b : config->battle_start_params.buildings)
	{
		...
		// 根据枚举类型调用 UnitFactory 的不同方法
		switch (b.type)
		{
		case BuildingType::CANNON:
			unit = UnitFactory::CreateCannon(b.level, game_world_, background_sprite_);
			break;
...
void BattleTestLayer::placeUnitAt(float gridX, float gridY)
{
	...
	{
		showDeployHintAtWorldPos(hintLocalPos); // 现在的逻辑里这个函数会识别 NONE 并显示 "SELECT A UNIT FIRST!"
		return;
	}

	if (!battle_manager_->CanDeployUnit(current_selected_unit_))
	{
	...
	{
		BattleUnit* unit = createUnitByType(current_selected_unit_, 1);
		if(unit)
		{
			unit->SetPositionAttacker(gridX, gridY);
			battle_manager_->AddUnit(std::unique_ptr<BattleUnit>(unit), true);
			//扣除数量并更新面板
			battle_manager_->decrementUnitCount(current_selected_unit_);
			int remain = battle_manager_->getRemainingUnitCount(current_selected_unit_);
			unit_selection_panel_->updateUnitCount(current_selected_unit_, remain);
		}
	}
	else
	{
		BattleUnit* tempUnit = createUnitByType(current_selected_unit_, 0);
		if (tempUnit)
		{
			...
			auto cleanup = CallFunc::create([tempUnit]()
				{
					delete tempUnit;
				});
			this->runAction(Sequence::create(delay, cleanup, nullptr));
		}
		CCLOG("Click out of bounds: %.2f, %.2f", gridX, gridY);
		return;
	}
}

//根据类型创建单位的辅助函数
BattleUnit* BattleTestLayer::createUnitByType(UnitType type, int level)
{
	switch (type)
	{
	case UnitType::BARBARIAN:
		return UnitFactory::CreateBarbarian(level, game_world_, background_sprite_);

	case UnitType::ARCHER:
		return UnitFactory::CreateArcher(level, game_world_, background_sprite_);
