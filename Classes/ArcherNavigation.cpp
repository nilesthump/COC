...
ArcherNavigation::ArcherNavigation()
{}

ArcherNavigation::~ArcherNavigation()
{}

BattleUnit* ArcherNavigation::FindTarget(BattleUnit* self,
...
//这里没有用A*，直接朝目标移动
void ArcherNavigation::CalculateMove(BattleUnit* self, BattleUnit* target, float deltaTime)
{
    if (!target || !self)
        return;
    ...
    // 如果已经非常接近目标，就不需要移动
    if (distance < self->GetAttackDistance())
    {
        return;
    }
...
bool ArcherNavigation::IsInAttackRange(BattleUnit* self, BattleUnit* target)
{
...
std::string ArcherNavigation::GetNavigationType() const
{
    return "Barbarian";
}

//欧几里得距离
float ArcherNavigation::CalculateDistance(BattleUnit* a, BattleUnit* b)
{
    if (!a || !b)
